...
class MarkdownLogger:
    ...
    def create_file(self):
        self.records = pd.DataFrame(
            columns=["Solved", "Time(s)", "Solution", "Language"],
    ...
    def read_file(self, src_path=str):
        ...
        for line in self.records:
            corr_records = np.append(
                corr_records, [re.sub(white_patt, "", line).split("|")[1:-1]], axis=0
            )

        self.records = pd.DataFrame(
            corr_records[1:], columns=columns, index=range(1, self.last_problem + 1)
        )
    ...
    def add_problem(self, solution, problem_id=int, duration=float, language="Python"):
        self.records = self.records.loc[
            :, ["Solved", "Time(s)", "Solution", "Language"]
        ]
        self.records.index.name = "Problem"
